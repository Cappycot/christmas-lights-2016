    ...
    try:
            ...
            for line in data:
                ...
                if line.lower().startswith("section:"):
                    ...
                    if line.lower().startswith("time:"):
                        ...
                        line = line[1:-1].split(",")
                        section.entries.append(Entry(int(line[0]), float(line[1]), float(line[2])))
        ...
        for section in sections:
            ...
            for i in range(0, section.repeat):
                ...
                for old in section.entries:
                    entries.append(Entry(old.channel, old.start + offset, old.duration))
            for time in section.times:
                ...
                for entry in entries:
                    ...
                    events.append(Event(start + entry.duration * 60 / section.bpm, Event.Dark, entry))
        ...
        for event in events:
                ...
                for i in range(0, 8):
                    ...
                    if ch == 1:
                        ...
                        if last[i] == 0:
                            ...
                            #lit += 1
                #if lit >= 6:
                    #print("Warning! The relay probably won't be able to light all of this here.")
                    #toreturn += "Warning! The relay probably won't be able to light all of this here.\n"
                lserial = lserial[:-1]
                if len(sections) > 0:
                    out.write("a" + str(cur))
                    for sec in sections:
            ...
            if event.type == Event.Print:
                ...
                cut_offs[entry.channel - 1] = event.time + entry.duration * 60 / bpm
        ...
        if not wascompiled:
            ...
            out.close()
        song.compiled = True
        toreturn += "Song successfully " + ("re" if wascompiled else "") + "compiled."
    except:
        e = exc_info()
...
def init_serial():
    global ser
    if not ser is None:
        return "Serial already initialized."
    try:
        ser = serial.Serial(
            port='/dev/ttyACM0',
            baudrate=115200
        ...
        return "Serial connection could not be initialized."
    ...
    try:
            ...
            if line == "w0":
                ...
                if ser is None:
                    if line.startswith("0") or line.startswith("1"):
                        line = line.replace("0", "l").replace("1", "d").replace(",", test_space)
                        line = line.replace("l", test_light_color).replace("d", test_dim_color)
                        line += test_endl
                steps.append(line)
        data.close()
        print("Loading music...")
        ...
        for step in steps:
            if step.startswith("w"):
                ...
                mark = elapsed - mixpos
                cur = time.time()
                print("Lightmap time: " + str(int(elapsed * 1000) / 1000) + "; pygame time: " + str(int(mixpos * 1000) / 1000))
                ...
                elif -mark > lag_tolerance and int((cur - last_correction) * 1000) / 1000 > 2:
                    last_currection = cur
                    lag_debt = -mark * catch_up_multiplier
                    print(str(lag_debt) + " seconds will be shaved off the next alignment time.")
                elapsed = align
                continue
...
def scan_songs():
    ...
    return "Found " + str(len(songs)) + " song" + ("" if len(songs) == 1 else "s") + "."
    ...
    if query.startswith("quit"):
        ...
        try:
            ...
            if bluetooth_delay < 0:
                volume = 0
            result = "Bluetooth time delay set to " + str(bluetooth_delay) + " seconds."
        except:
            result = "Invalid time parameter."
    elif query.startswith("bluetooth"):
        bluetooth = not bluetooth
        if bluetooth:
            result = "Bluetooth time delay is on at " + str(bluetooth_delay) + " seconds."
        else:
        ...
        result = "\033[1m\033[32mSongs:\n\033[0m" if len(songs) > 0 else "There are no songs.\n"
        for song in songs:
            titlematch = song.name != song.title
            if titlematch:
                result += "\""
            result += song.name + (("\" " + song.title) if titlematch else "") + "\n"
        result = result[:-1]
